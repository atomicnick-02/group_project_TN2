...
#         self.Q = np.eye(2 * self.model.model.nv) * 30
#         self.R = np.eye(self.model.model.nu) * .08
#         self.QN = np.eye(2 * self.model.model.nv)
...
#             self.x_bar = np.zeros(self.model.model.nv)
#             self.u_bar = self.model.get_gravity_vector()

#             self.lqr = LQRController(self.Q,self.R,self.QN,self.x_bar,self.u_bar)
#             A, B = self.model.get_linerazation_matrices(self.x_bar,self.u_bar)
#             A,B = self.model.discritize_matrices(A,B)
#             self.lqr.compute_gains(A, B)
...
#                 start_time = 0.0
#                 end_time = 0.5
#                 q_target, q_dot_target, q_ddot_target = self.traj_opt.qubic_splines(t, q_init, q_dot_init, q_des, q_dot_des, start_time,
#                                                                            end_time)
...
#                 u_new = self.lqr.compute_control(x_state,x_ref)
